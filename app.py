...
def card(title: Optional[str] = None, note: Optional[str] = None, key: Optional[str] = None):
    """A white card for one section of a page. Use as `with card("Title", "note"):`."""
    box = st.container(border=True, key=f"card-{key or slug(title or 'card')}")
    if title:
        box.markdown(f"<p class='card-title'>{title}</p>", unsafe_allow_html=True)
    if note:
        box.markdown(f"<p class='card-note'>{note}</p>", unsafe_allow_html=True)
    return box
...
def download_button(df: pd.DataFrame, label: str, filename: str, key: str) -> None:
    ...
    # Tertiary: a quiet text link, so the charts keep the visual weight.
...
def page_system_map(df: pd.DataFrame) -> None:
    ...
    with card(
        ...
        chart_type = c2.segmented_control("Chart", ["Treemap", "Sunburst"], default="Treemap",
                                          key="v1_chart") or "Treemap"
    ...
    with card(
        ...
        with st.expander("All subcategories with counts", icon=":material/table_rows:"):
            ...
            download_button(coverage.drop(columns=["samples"]), "Download coverage table",
                            "subcategory_coverage.csv", key="v1_dl_cov")
    ...
    with card(
        ...
        view_as = st.segmented_control("Show", ["Count", "Share of domain"], default="Count",
                                       key="v1_pop_view") or "Count"
        event = plot(
...
def peers_for_organization(df: pd.DataFrame) -> None:
    ...
    with card("Peer matrix", "Ranked by shared subcategories. Select a row to compare the two "
              "organizations' outcomes.", key="peer_matrix"):
    ...
    with card(f"{org} and {peer}", f"Their outcomes in the {len(shared)} goals they share."
              + ("" if rows else " Showing the top peer; select another row above to switch."), key="peer_pair"):
    ...
    with card("Overlap heatmap", "How many outcomes each peer has in each of this organization's goals. "
              "Hover a cell to read them.", key="peer_heatmap"):
...
def page_review(df: pd.DataFrame) -> None:
    ...
    with card("How sure the coder was", "Each outcome's confidence rating. The list below opens on the "
              "low and no-confidence ones.", key="review_filters"):
...
def welcome_screen() -> None:
    ...
    with card("Upload a coded outcomes export", key="welcome"):
        st.markdown(
...
def sidebar(df: pd.DataFrame, source: str, page_key: str) -> pd.DataFrame:
    ...
    with st.sidebar:
        st.markdown(f"<p class='brand'>{APP_TITLE}</p><p class='brand-note'>{source} · "
                    f"{len(df):,} outcome statements</p>", unsafe_allow_html=True)
        # Filters show first; settings sit in a drawer below them but must run
        # first, because the organization grouping changes the filter options.
    ...
    with filters_box:
        ...
        chosen_pops = st.multiselect("Target populations", pops, format_func=pretty_population,
                                     key="f_pops", placeholder="All populations", label_visibility=hidden)
        ...
        if page_key != "review":
            ...
            chosen_conf = st.multiselect(
                ...
                format_func=str.title, label_visibility=hidden,
            )
        ...
        if active:
            ...
            st.button("Reset filters", on_click=reset_filters, icon=":material/filter_alt_off:", type="tertiary")
        st.write("")
